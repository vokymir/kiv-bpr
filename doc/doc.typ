...
ensures dissipation on any graph with either single vertex with the highest
degree (higher than 0) or multiple highest degree vertices which are not the
only vertices of one graph component, the rule being:
...
/ Fill up:
  Single highest degree vertex or multiple highest degree vertices which are
  not the only vertices of one graph component.
